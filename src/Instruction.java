		...
		if(!inst.isEmpty()){
			...
			// save命令の実行
				...
				if(inst.get(1).equals("TABLE")){
					...
					if(table.isEmpty()){
						System.out.println("Table \"" + inst.get(1) + "\" don't exist.");
						return table;
					}
				// テーブルがテーブル名で与えられた場合，テーブル集合からテーブルを取得する
				}else{
					if(database.getTableNames().contains(inst.get(1).toLowerCase())){
			...
			// JOIN文の実行
				...
				if(inst.get(i-1).equals("TABLE")){
					...
					sub++;
					if(table1.isEmpty()){
						System.out.println("Table \"" + inst.get(i-1) + "\" don't exist.");
						return table1;
					}
				// テーブルがテーブル名で与えられた場合，テーブル集合からテーブルを取得する
				}else{
					if(database.getTableNames().contains(inst.get(i-1).toLowerCase())){
				...
				// テーブルがテーブル名で与えられた場合，テーブル集合からテーブルを取得する
					...
					if(database.getTableNames().contains(inst.get(i+1).toLowerCase())){
						...
						System.out.println("Table \"" + inst.get(1+1) + "\" don't exist.");
						return table2;
					}
				}
